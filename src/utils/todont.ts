...
const createMobileIcon = (id: string, iconType: string): HTMLButtonElement => {
  ...
  iconButton.id = id;
  iconButton.className = "bp3-button bp3-minimal rm-mobile-button dont-unfocus-block";
...
export const replaceText = ({
  ...
  if (diff !== 0) {
    let index = 0;
    const maxIndex = Math.min(Math.max(oldValue.length, text.length), Math.max(start, end) + 1);
...
const initializeTodont = (extensionAPI: OnloadArgs["extensionAPI"]) => {
  ...
  return (todontMode: (typeof TODONT_MODES)[number]) => {
    // Clean up previous mode before setting up new one
    unloads.forEach((u) => u());
    unloads.clear();

    const configuredHotkey = ((extensionAPI.settings.get("todont-hotkey") as string) || "").trim();
    extensionAPI.ui.commandPalette.addCommand({
      label: ARCHIVE_COMMAND_LABEL,
      callback: todontCallback,
      "default-hotkey": configuredHotkey || "ctrl+shift+enter",
    ...
    const styleArchivedButtons = (node: HTMLElement) => {
      ...
      Array.from(buttons).forEach((button) => {
        if (button.innerText === "ARCHIVED" && button.className.indexOf(TODONT_CLASSNAME) < 0) {
          button.innerText = "x";
    ...
    const iconObserver = createObserver((mutationList: MutationRecord[]) => {
      ...
      if (!!mobileBackButton && !document.getElementById("mobile-todont-icon-button")) {
    ...
    if (todontMode === "strikethrough") {
      ...
      const strikethroughObserver = createHTMLObserver({
        callback: (b: HTMLButtonElement) => {
          ...
          const block = CLASSNAMES_TO_CHECK.map((c) => b.closest(`.${c}`) as HTMLElement).find(
            (d) => !!d,
          );
          if (block) {
            styleBlock(block);
