...
const deferTodos = (blockUid: string) => {
  if (!blockUid) return;
    ...
    if (isNaN(howManyDaysInt)) {
      ...
      switch (howManyDays) {
        ...
        case "tm":
          ...
          var endOfNextMonth = endOfMonth(addMonths(rmDateParse, 1)).getDate();
...
const addDeferTODOsCommand = () => {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (target.nodeName === "BUTTON" && target.innerText === "Defer") {
      const blockUid = getBlockUidFromTarget(target);
      deferTodos(blockUid);
    }
  });
};
